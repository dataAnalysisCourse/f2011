function [opsinTracking pByFreq] = compareOpsinTracking(patchData, dataDirectory)
...
for iOpsin = 1:nOpsins
	...
	opsinTracking(iOpsin).nSpikesEvokedByCell = nSpikesEvokedThisOpsin;
...
% Compute a two-sample t-test across opsins at each frequency
if any(round(opsinTracking(1).pulseFrequencyHz) ~= ...
	   round(opsinTracking(2).pulseFrequencyHz))
	error('Light pulse frequencies used differ between the two opsins');
end

[~, pByFreq] = ttest2(opsinTracking(1).nSpikesEvokedByCell, ...
					  opsinTracking(2).nSpikesEvokedByCell);
...
for iOpsin = 1:nOpsins
	...
	% figure out where to put the asterisks if we need them
	xPos = opsinTracking(1).pulseFrequencyHz;
	yPos = max([opsinTracking(1).spikesEvokedMean + opsinTracking(1).spikesEvokedSEM; ...
				opsinTracking(2).spikesEvokedMean + opsinTracking(2).spikesEvokedSEM],[], 1) + ...
		   diff(get(gca, 'YLim'))*0.05; % add 5% of the height of the figure as margin

	for iFreq = 1:length(pByFreq)
		if pByFreq(iFreq) < 0.001
			str = '***';
		elseif pByFreq(iFreq) < 0.01
			str = '**';
        elseif pByFreq(iFreq) < 0.05
            str = '*';
        else
			% not significant, go the next frequency
			continue;
		end

		text(xPos(iFreq), yPos(iFreq), str, ...
