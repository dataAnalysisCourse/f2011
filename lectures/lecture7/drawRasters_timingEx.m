% drawRasters_timingEx.m
%
% Modified for use in Lecture 07
%
...
%
% USAGE:
%  axh = drawRastersBad( axh, spikeTimes, xlim, verticalOffset )
%
% INPUTS:
...
function axh = drawRasters_timingEx( axh, spikeTimes, xlim, verticalOffset )
    % ******************************************************************
    %                    Parameter Check
    % ******************************************************************
    tickHeight = 5;
...
%             lineh = line( [mySpikeTimes(iSpike) mySpikeTimes(iSpike)], myY, 'Parent', axh, ...
%                 'Color','k','LineWidth', 1);
%             lineHandles(end+1) = lineh;
%         end %for iSpike = 1 : length( mySpikeTimes ) 
        %Profiler tells us that with this way, drawRastersBad takes ~.24s,
        %or 24% of total time.
        
        % Faster way:
        line( [mySpikeTimes' mySpikeTimes'], myY, 'Parent', axh, ...
            'Color','k','LineWidth', 1);
        % Profiler tells us that with this way, drawRastersBad takes ~.16s,
        % or 17% of total time.
